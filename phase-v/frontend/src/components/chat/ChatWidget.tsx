...
import { getKafkaClient, disconnectKafkaClient } from '@/lib/kafka-client'
 ...
 * T100: Includes WebSocket connection status indicator.
 */
export default function ChatWidget({ userId }: ChatWidgetProps) {
  ...
  const [isConnected, setIsConnected] = useState(false)  // T100: WebSocket connection status
  const messagesEndRef = useRef<HTMLDivElement>(null)

  /**
   * T100: Initialize WebSocket connection status tracking.
   */
  useEffect(() => {
    ...
    const unsubscribe = client.onConnectionStatusChange((connected: boolean) => {
      ...
      <AnimatePresence mode="wait">
        {!isOpen && (
          ...
          >
            ...
            {/* T100: WebSocket connection status indicator */}
            <motion.div
