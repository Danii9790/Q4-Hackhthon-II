 ...
 * - T099: Real-time task updates via Kafka WebSocket
 */
...
import { getKafkaClient, type TaskUpdateMessage } from '@/lib/kafka-client'
...
interface TaskListProps {
  ...
  onTasksUpdate?: () => void  // T099: Callback to refresh tasks
}
...
export default function TaskList({
  ...
  const clientRef = useRef<ReturnType<typeof getKafkaClient> | null>(null)

  // T099: Initialize real-time updates
  useEffect(() => {
    // Get current user
    const user = getCurrentUser()
    if (!user?.id || !onTasksUpdate) {
      return
    }
    ...
    const unsubscribe = client.onTaskUpdate((message: TaskUpdateMessage) => {
      console.log('Real-time task update received:', message)

      // Refresh task list when any update occurs
      onTasksUpdate()
    })
